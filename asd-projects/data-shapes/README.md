...
3. **Find where TODO 3b is located in your code.**
   - Call your `handleStatic` function here, passing in the current shape object. The `dataShapes` array holds all the shape objects. The variable `currentIndex` (provided for you) keeps track of which shape is currently selected as an index number for the `dataShapes` array.
